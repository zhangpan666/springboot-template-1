...
public class AdminSysUserServiceImpl implements AdminSysUserService {
    ...
    @Override
    public CommonResult<List<SysUserVO>> listSysUser(Page page, SysUser param) {
        PageInfo<SysUser> pageInfo = PageUtil.query(page, param, sysUserService::query);
        List<SysUser> list = pageInfo.getList();
        list.forEach(sysUser -> {
            Boolean hasKey = RedisStringHashUtil.hasKey(ProjectConstant.RedisKey.ONLINE_ADMIN.getKeyPrefix(), sysUser.getUsername());
            if (hasKey){
                sysUser.setOnline(ProjectConstant.COMMON_STATUS_AVAILABLE);
            } else {
                sysUser.setOnline(ProjectConstant.COMMON_STATUS_NOT_AVAILABLE);
            }
        });
        List<SysUserVO> sysUserVOList = list.stream().map(sysUser -> {
            SysUserVO sysUserVO = new SysUserVO().setId(sysUser.getId()).setNickname(sysUser.getNickname()).setUsername(sysUser.getUsername()).setOnline(sysUser.getOnline())
                    .setLastLoginTime(sysUser.getLastLoginTime()).setRecentLoginTime(sysUser.getRecentLoginTime()).setCreateTime(sysUser.getCreateTime())
    ...
    @Override
    public CommonResult<SysUserVO> getSysUserDetail(Long id) {
        SysUser sysUser = sysUserService.getById(id);
        if (sysUser==null){
            return CommonResult.businessWrong("该系统用户不存在");
        }
        SysUserVO sysUserVO = new SysUserVO().setId(sysUser.getId()).setNickname(sysUser.getNickname()).setCreateTime(sysUser.getCreateTime())
                .setUpdateTime(sysUser.getUpdateTime());
    ...
    @Override
    public CommonResult saveSysUser(SysUser sysUser, SysUserRole userRole) {
    ...
    @Override
    public CommonResult deleteSysUser(Long id) {
        SysUser sysUser = sysUserService.getById(id);
        String username = sysUser.getUsername();
        Subject subject = SecurityUtils.getSubject();
        String loginUserName = subject.getPrincipal().toString();
    ...
    @Override
    public boolean authSysUser(SysUserRole userRole) {
    ...
    public CommonResult changePassword(String oldPassword, String newPassword) {
        ...
        if (update) {
            OperationRecordUtil.record(null, "系统用户", "修改密码");
            subject.logout();
            return CommonResult.success();
        }
        return CommonResult.businessWrong();
    ...
    public void generateGoogleAuthQrCode(String username, HttpServletRequest request, HttpServletResponse response) throws RuntimeException {
        ...
        String issuer = "49开奖:" + username + "的登录验证码";
        String account = "https://manage.49kjapi-sdk.com";
    ...
    @Override
    public CommonResult<List<RoleVO>> listRole() {
        List<Role> roleList = roleService.list();
        List<RoleVO> roleVOList = roleList.stream().map(role -> {
            RoleVO roleVO = new RoleVO().setId(role.getId()).setCreateTime(role.getCreateTime()).setRoleName(role.getRoleName()).setRoleCode(role.getRoleCode())
    ...
    @Override
    public CommonResult<List<SysUserRoleVO>> listSysUserRole(SysUserRole param, Page page) {
        PageInfo<SysUserRole> pageInfo = PageUtil.query(page, param, sysUserRoleService::query);
        List<SysUserRole> list = pageInfo.getList();
        List<SysUserRoleVO> sysUserRoleVOList = list.stream().map(sysUserRole -> {
            SysUserRoleVO sysUserRoleVO = new SysUserRoleVO().setId(sysUserRole.getId()).setCreateTime(sysUserRole.getCreateTime()).setNickname(sysUserRole.getNickname())
                    .setRoleCode(sysUserRole.getRoleCode()).setRoleId(sysUserRole.getRoleId()).setRoleName(sysUserRole.getRoleName())
                    .setStatus(sysUserRole.getStatus()).setUpdateTime(sysUserRole.getUpdateTime()).setUsername(sysUserRole.getUsername());
            return sysUserRoleVO;
        }).collect(Collectors.toList());
        return CommonResult.success(sysUserRoleVOList);
    }
}
